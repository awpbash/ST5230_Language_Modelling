...
\title{\textbf{ST5230 Assignment 1}\\Language Modeling, Embeddings, and Downstream Classification}
\author{Ng Jun Wei (A0252142U)\\National University of Singapore}
\date{}
...
\caption{Validation perplexity by epoch. The Transformer converges fastest and reaches the lowest perplexity. All three neural models continue improving at epoch 20, with no sign of overfitting (Figure~2).}
\end{figure}
...
\item \textbf{Frozen-only embeddings in Part II:} The ablation compares frozen pretrained vs.\ trainable scratch, but does not test \emph{fine-tuned} pretrained embeddings (initialize from W2V/GloVe then allow gradient updates), which could combine the benefits of both approaches.
\end{itemize}
